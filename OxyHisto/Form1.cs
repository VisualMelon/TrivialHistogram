...
namespace OxyHisto
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs eargs)
        {
            ...
            model.Axes.Add(new LinearAxis { Position = AxisPosition.Left, MajorGridlineStyle = LineStyle.Solid, Key = "YBottom", StartPosition = 0, EndPosition = 0.48, AbsoluteMinimum = 0, AbsoluteMaximum = 5, Maximum = 5 });
            model.Axes.Add(new LinearAxis { Position = AxisPosition.Left, MajorGridlineStyle = LineStyle.Solid, Key = "YTop", StartPosition = 0.52, EndPosition = 1.0, AbsoluteMinimum = 0, AbsoluteMaximum = 5, Maximum = 5 });
            model.Axes.Add(new LinearAxis { Position = AxisPosition.Bottom });

            var chs1 = new ContinuousHistogramSeries() { YAxisKey = "YBottom" } ;
            chs1.ItemsSource = HistogramHelpers.Collect(RandomSource(10000), 0, 1, 10, true);
            chs1.StrokeThickness = 1;
            model.Series.Add(chs1);

            var chs2 = new ContinuousHistogramSeries() { YAxisKey = "YTop" } ;
